...
function ensureQueueInitialized(): boolean {
  ...
  try {
    ...
    logger.warn(`[WebhookQueue] Failed to initialize Producer: ${err instanceof Error ? err.message : String(err)}`);
...
export async function enqueueWebhookDlq(payload: WebhookDeliveryPayload, failedReason: string | undefined): Promise<Job<WebhookDeliveryPayload> | null> {
  ...
  try {
    const job = await webhookDlqInstance.add('webhook-dlq', payload, {
      jobId: `dlq-${payload.merchantId}-${payload.invoiceId}-${Date.now()}`,
    });
    logger.info(`[WebhookQueue] Enqueued job ${job.id} into DLQ for merchant ${payload.merchantId}. Reason: ${failedReason}`);
    return job;
  } catch (err) {
    logger.error(`[WebhookQueue] DLQ add failed: ${err instanceof Error ? err.message : String(err)}`);
    return null;
  }
}
...
export async function enqueueWebhook(payload: WebhookDeliveryPayload): Promise<Job<WebhookDeliveryPayload> | null> {
  if (!ensureQueueInitialized() || !webhookQueueInstance) {
    logger.warn('[WebhookQueue] Queue not available — webhook fallback: %s %s', payload.eventType, payload.invoiceId);
  ...
  try {
    job = await webhookQueueInstance.add('webhook-delivery', payload, {
    ...
    logger.warn('[WebhookQueue] Queue add failed — webhook fallback: %s %s', payload.eventType, payload.invoiceId);
    await prisma.webhookDelivery.create({
      data: {
        merchantId: payload.merchantId,
        eventType: payload.eventType,
        payload: payload as unknown as Prisma.InputJsonValue,
        status: 'failed',
        error: `Webhook queue add failed: ${err instanceof Error ? err.message : String(err)}`,
        completedAt: new Date(),
      },
    }).catch((dbErr: unknown) => {
      logger.error({ err: dbErr }, '[WebhookQueue] Failed to persist fallback webhook record');
